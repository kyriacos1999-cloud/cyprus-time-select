    ...
    <Sheet open={open} onOpenChange={setOpen}>
      <SheetTrigger asChild>
        <button className="relative text-primary-foreground hover:text-primary-foreground/80 transition-colors">
          ...
          {itemCount > 0 && (
            <span className="absolute -top-2 -right-2 bg-[hsl(var(--rolex-gold))] text-foreground text-[9px] font-bold w-4 h-4 rounded-full flex items-center justify-center">
              {itemCount}
            </span>
          )}
        </button>
          ...
          <>
            <div className="flex-1 overflow-y-auto space-y-4 py-4">
              {items.map((item) => {
                ...
                return (
                  <div key={`${item.product.id}-${item.withBox}`} className="flex gap-3 border border-border p-3">
                    <img
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-sm font-display text-foreground mt-1">€{price}</p>
